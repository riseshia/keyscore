...
- [x] **Grader 타이밍 보정** — 적응형 오프셋으로 구현 (PR #16)
- [x] **구간 반복 연습** — 시작/끝 beat 지정, 해당 구간만 판정, 리셋 버튼 (PR #18)
- [x] **BPM 조절** — 배속 변경 0.1 단위 (0.1x~1.0x), 타이밍 윈도우 비례 보정 (PR #19)
- [ ] **구간 연습 자동 반복** — 구간 완료 → 커서 자동 복귀 → 올바른 건반 입력 시 색칠/스코어 리셋 + 재시작
- [ ] **Miss/Error 색상 분리** — Error를 회색으로 변경 (Good이 주황 계열이므로)
- [ ] **음표 클릭으로 구간 지정** — OSMD SVG 클릭 → beat index 매핑
- [ ] **진행 바** — 상단에 얇은 바로 현재 beat / 전체 beats 비율 표시
- [ ] **MIDI 자동 연결** — 디바이스 1개일 때 자동 선택
- [ ] **실효 BPM 표시** — 배속 옆에 원곡 BPM 기반 실효 BPM 병기 (예: `0.7x (84 BPM)`)
- [ ] **iPad Safari 대응** — File System Access API 미지원, `<input type="file">` fallback
- [ ] **연습 이력 저장** (localStorage) — 곡별 정확도 추이
- [ ] **Error 원인 표시** — 리뷰에서 Error 음표에 기대 음 vs 입력 음 표시 (MIDI noteOn pitch 기록)
- [ ] 폴링 노트 모드 (악보 암기 후 타이밍 연습용)
- [ ] MIDI 파일 입력 지원 (폴링 노트용)
- [ ] 오디오 재생
- [ ] Wait 모드
- [ ] 리뷰 모드 마디 네비게이션 (M3에서 이동)
